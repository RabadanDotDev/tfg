...
CSV_FOLDER = Path("./tmp/toniot")
TABLE_RESULT_TREE = Path("./report/theoretical_framework/datasets_toniot_protos.tex")
TABLE_RESULT_IP = Path("./report/theoretical_framework/datasets_toniot_protosip.tex")
...
def get_testbed_ips(conversations: pd.DataFrame) -> dict:
    ...
    res["internal_ips_total"] = list(pd.concat([
        conversations[conversations['ip origin'].str.contains("192.168.1.")]['ip origin'],
        conversations[conversations['ip dest'].str.contains("192.168.1.")]['ip dest'],
    ]).unique())
...
def plot_histograms(conversations: pd.DataFrame):
    ...
    plt.savefig(REPORT_MEDIA_FOLDER / f'./toniot_pcap_duration_distribution.png', bbox_inches="tight")
    ...
    plt.savefig(REPORT_MEDIA_FOLDER / f'./toniot_pcap_bytes_distribution.png', bbox_inches="tight")
    ...
    plt.savefig(REPORT_MEDIA_FOLDER / f'./toniot_pcap_frames_distribution.png', bbox_inches="tight")
...
def conversation_evaluate():
    ...
    dump_as_json(RESULT_FOLDER / 'toniot_unique_ips.json', unique_ips)
    dump_as_json(RESULT_FOLDER / 'toniot_testbed_ips.json', testbed_ips)
...
def tree_proto_gen_tex_table(contents):
    ...
    lines.append(r'    \caption{Primeras tres capas de protocolos identificados en TON-IoT}')
    lines.append(r'    \label{table:toniotprotocols}')
...
def treeproto_evaluate_files():
    ...
    dump_as_json(RESULT_FOLDER / 'toniot_tree_proto_combined.json', contents)
...
def ip_proto_gen_tex_table(contents):
    ...
    lines.append(r'        \end{tabular}')
    lines.append(r'    \end{center}')
    lines.append(r'    \caption{Protocolos identificados analizando exclusivamente la capa IP en TON-IoT}')
    lines.append(r'    \label{table:toniotprotocolsip}')
    lines.append(r'\end{table}')
...
def ipproto_evaluate_files():
    ...
    dump_as_json(RESULT_FOLDER / 'toniot_ip_proto_combined.json', contents)
